...
# note that This is created by kavya trivedi
# if you are adding new things then comit like {what that part do and in brackets (yourname)}
# Database file
...
def ai_food_matching(data: Dict[str, Any]) -> None:
    donations = [f"{d['item']} {d['quantity']}" for d in data['donations'].values() if not d['claimed']]
    ngo_needs = [f"{u['name']} needs food" for u in data['users'].values() if u['role'] == 'NGO']

    if not donations or not ngo_needs:
        print("Not enough data for matching.")
        return

    vectorizer = TfidfVectorizer()
    tfidf_matrix = vectorizer.fit_transform(donations + ngo_needs)

    cosine_similarities = cosine_similarity(tfidf_matrix[:len(donations)], tfidf_matrix[len(donations):])

    for i, donation in enumerate(donations):
        best_match = np.argmax(cosine_similarities[i])
        print(f"Suggested match: {donation} -> {ngo_needs[best_match]}")
